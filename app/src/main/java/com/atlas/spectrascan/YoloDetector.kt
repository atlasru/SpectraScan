...
internal data class YoloDetection(
    val classId:Int,val label:String,val confidence:Float,val normalizedBox:RectF,
...
internal class YoloDetector(context:Context):AutoCloseable{
    ...
    @Synchronized fun detect(bitmap:Bitmap,filter:TargetFilter,gain:Float=1f):Pair<List<YoloDetection>,Int>{
        ...
        val scale=min(INPUT_SIZE/srcW,INPUT_SIZE/srcH);val drawW=srcW*scale;val drawH=srcH*scale;val padX=(INPUT_SIZE-drawW)/2f;val padY=(INPUT_SIZE-drawH)/2f
        scaledCanvas.drawColor(Color.rgb(114,114,114));scaledCanvas.drawBitmap(bitmap,null,RectF(padX,padY,padX+drawW,padY+drawH),scalePaint);scaledBitmap.getPixels(pixels,0,INPUT_SIZE,0,0,INPUT_SIZE,INPUT_SIZE)
        ...
        tensor.use{input->session.run(mapOf(inputName to input)).use{result->val batch=result[0].value as? Array<*> ?: return@use;val channels=batch.firstOrNull() as? Array<*> ?: return@use;if(channels.size<84)return@use
            ...
            for(i in 0 until anchors){var bestClass=-1;var bestScore=0f;for(classId in COCO_LABELS.indices){val scores=channels[4+classId] as? FloatArray ?: continue;if(i>=scores.size)continue;val s=scores[i];if(s>bestScore){bestScore=s;bestClass=classId}}
                ...
                val leftPx=(boxX[i]-boxW[i]/2f-padX)/scale;val topPx=(boxY[i]-boxH[i]/2f-padY)/scale;val rightPx=(boxX[i]+boxW[i]/2f-padX)/scale;val bottomPx=(boxY[i]+boxH[i]/2f-padY)/scale
        ...
        val refined=kept.map { detection ->
            val r=RoiMaskRefiner.refine(bitmap,detection)
            if(r.mask.isEmpty()) detection else detection.copy(normalizedBox=r.box,maskCells=r.mask,maskQuality=r.quality)
        }
        return refined to rejected
    }
